...
async function checkProjectExists({ aliasId, apiKey, project, user }) {
    if (aliasId && apiKey === API_KEY && project && user) {
        return await checkFile(aliasId, project, user);
    }
    return {
        errorMessage: 'There was a problem with the export id, api key, project name, or user.',
        successMessage: null
    };
}
...
async function checkFile(aliasId, project, user) {
    try {
        const params = {
